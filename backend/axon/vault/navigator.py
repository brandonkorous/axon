...
class MemoryNavigator:
    ...
    def graph(self) -> VaultGraph:
        if self._cache:
            return self._cache.build_graph()
    ...
    def _search_and_rank(self, query: str, token_budget: int) -> RetrievedContext:
        ...
        # Boost/penalize based on learning metadata (confidence, validations)
        self._apply_learning_boost(scores, file_contents)
    ...
    def _apply_graph_boost(self, scores: dict[str, float]) -> None:
        ...
        for path, score in list(scores.items()):
            ...
            if node:
                ...
                scores[path] = score + min(hub_score, 2.0)
    ...
    def _apply_learning_boost(
        self, scores: dict[str, float], file_contents: dict[str, str],
    ) -> None:
        """Boost or penalize scores based on learning metadata."""
        for path in list(scores.keys()):
            ...
            if isinstance(validated, list) and validated:
                scores[path] += min(len(validated) * 0.5, 2.0)

            source_convos = metadata.get("source_conversations")
            if source_convos and int(source_convos) > 3:
                scores[path] += 1.0
